...
def _build_features_page(config: dict[str, Any]) -> None:
    ...
    if preview is not None and summary is not None:
        ...
        st.dataframe(preview.tail(10).reset_index(), use_container_width=True)
        ...
        if selected:
            st.line_chart(preview[selected], use_container_width=True)
        ...
        if label_counts:
            ...
            st.bar_chart(label_frame, use_container_width=True)
...
def _train_model_page(config: dict[str, Any]) -> None:
    ...
    cv_cache_bust = datetime.utcnow().isoformat() if cv_force else "stable"
    ...
    run_cv = st.button("Run CV", use_container_width=True, disabled=bool(cv_errors))
    ...
    train_cache_bust = datetime.utcnow().isoformat() if train_force else "stable"
    ...
    if isinstance(importance, dict) and importance:
        ...
        st.bar_chart(sorted_importance, use_container_width=True)
...
def _predictions_page(config: dict[str, Any]) -> None:
    ...
    with action_col:
        run_batch = st.button("Batch Run", use_container_width=True)
    with realize_col:
    ...
    if refresh:
        st.session_state["predictions_cache_bust"] = datetime.utcnow().isoformat()
    ...
    with metrics_cols[0]:
        ...
        st.caption(
            f"Timestamp: {latest['timestamp_utc'].strftime('%Y-%m-%d %H:%M')} UTC"
        )
    with metrics_cols[1]:
        st.metric(
    ...
    st.line_chart(ts_indexed[["p_up"]], use_container_width=True)
    ...
    if realized_count:
        ...
        st.line_chart(rolling_hit, use_container_width=True)
    else:
...
def _backtest_page(config: dict[str, Any]) -> None:
    ...
    if refresh:
        st.session_state["backtest_cache_bust"] = datetime.utcnow().isoformat()
...
def _monitor_page(config: dict[str, Any]) -> None:
    ...
    with refresh_col:
        refresh = st.button("Monitor refresh", use_container_width=True)
        ...
        if not predictions_path.exists():
            ...
            with st.spinner("Refreshing live metrics..."):
                try:
                    ...
                    st.session_state["monitor_cache_bust"] = datetime.utcnow().isoformat()
    ...
    st.line_chart(aligned["close"], use_container_width=True)
    ...
    st.line_chart(realized["equity"], use_container_width=True)
