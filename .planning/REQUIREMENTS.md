...
- [x] **MODL-02**: Retraining/backtest windows are configurable and run as rolling walk-forward cycles (train window + backtest window) without manual intervention.
- [x] **MODL-03**: Regime/drift diagnostics are computed per retrain window and surfaced in model evaluation outputs.
