...
def test_api_defaults_match_config() -> None:
    ...
    assert _default_freq() == str(config["freq"]), (
        f"_default_freq() returned {_default_freq()!r}, " f"but config has freq={config['freq']!r}"
    )
...
def test_no_hardcoded_1h_4h_in_api_routes() -> None:
    ...
    for filename in route_files:
        ...
        assert 'Query("1h"' not in source, f'{filename} still contains hardcoded Query("1h" default'
        assert 'Query("4h"' not in source, f'{filename} still contains hardcoded Query("4h" default'
        assert (
            '= "1h"' not in source or "load_config" in source or "_default_" in source
        ), f'{filename} may contain hardcoded "1h" default not sourced from config'
        assert (
            '= "4h"' not in source or "load_config" in source or "_default_" in source
