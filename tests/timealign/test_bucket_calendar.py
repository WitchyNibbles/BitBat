...
def test_to_bar_accepts_five_minute() -> None:
    ...
    expected = pd.Series(
        base.floor("5min").tz_localize(None).to_numpy(),
        index=series.index,
        name=series.name,
    )
...
def test_bars_for_duration_basic() -> None:
    assert bars_for_duration("24h", "5m") == 288
    assert bars_for_duration("24h", "1h") == 24
    assert bars_for_duration("14h", "5m") == 168
    assert bars_for_duration("30d", "1h") == 720
